...
export function KanbanMusume({
  ...
  const [animation, setAnimation] = useState<"idle" | "bounce" | "wave">("idle")
  ...
  return (
    <div
      ...
      style={{
        ...
        left: position.x === 0 ? "20px" : "auto",
        transform: position.x !== 0 || position.y !== 0 ? `translate(${position.x}px, ${position.y}px)` : "none",
      }}
    >
      <div
        className={`relative transition-all duration-300 ${animation === "bounce" ? "animate-bounce" : animation === "wave" ? "animate-pulse" : ""}`}
      >
        {/* 消息气泡 */}
